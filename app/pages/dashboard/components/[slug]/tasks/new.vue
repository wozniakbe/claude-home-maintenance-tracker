...
async function handleSubmit(data: { title: string; description: string | null; dueAt: number | null }) {
  loading.value = true;
  error.value = null;

  try {
    await $csrfFetch(`/api/house-components/${slug}/tasks`, {
      method: "POST",
      body: data,
    });
    router.push(`/dashboard/components/${slug}`);
  }
  catch (e: unknown) {
    const fetchError = e as { statusMessage?: string };
    error.value = fetchError.statusMessage ?? "Failed to create task";
  }
  finally {
    loading.value = false;
  }
